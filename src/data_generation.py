...
def generate_dataset(
    ...
    and source.h5 for either train or test split. ``number_of_cases`` is the
    number of new cases to generate; ``start_case_id`` controls the first file
    ID so generation can safely append to an existing dataset. When supplied,
    ``number_of_cases`` overrides the split configuration.
    """
    ...
    if number_of_cases < 0:
        raise ValueError("number_of_cases must be non-negative.")
    n_damages = int(split_cfg["number_of_damages"])
    start_case_id = int(start_case_id)
    if start_case_id < 0:
        raise ValueError("start_case_id must be non-negative.")
    timings = []

    case_ids = range(start_case_id, start_case_id + number_of_cases)
    ...
    if not overwrite:
        ...
        if source_path.exists():
            existing_source = io.load_hdf(source_path)
            if existing_source.shape != source_values.shape or not np.array_equal(existing_source, source_values):
                raise FileExistsError(
                    f"Existing {source_path} is incompatible with the source generated by this config."
                )

    for case in tqdm(case_ids, desc=f"{split} cases"):
        timings.append(
